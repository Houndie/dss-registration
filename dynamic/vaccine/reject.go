...
func (s *Service) Reject(ctx context.Context, token, id, reason string) error {
	s.logger.Trace("vaccine reject service")
	...
	if err := s.store.ApproveVaccine(ctx, id, false); err != nil {
		return fmt.Errorf("error rejecting vaccine in store: %w", err)
	}

	exists, err := s.objectClient.Exists(ctx, id)
	if err != nil {
		return fmt.Errorf("error checking for vaccine proof existance: %w", err)
	}
