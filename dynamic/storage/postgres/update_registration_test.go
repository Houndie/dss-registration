...
func TestUpdateRegistration(t *testing.T) {
	...
	startRegistration := &storage.Registration{
		FirstName: "Joe",
		LastName:  "Dirt",
		Email:     "Hair@mullets.com",
		PassType:  &storage.NoPass{},
		Housing:   &storage.NoHousing{},
	}

	id, err := store.AddRegistration(context.Background(), startRegistration)
	if err != nil {
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			err := store.UpdateRegistration(context.Background(), test.registration)
			if err != nil {
				t.Fatalf("unexpected error updating registration: %v", err)
			}
...
func TestUpdateRegistrationDoesntExist(t *testing.T) {
	pool, err := pgxpool.Connect(context.Background(), os.Getenv("DSS_TEST_POSTGRESURL"))
	if err != nil {
		t.Fatalf("error connecting to database for test: %v", err)
	}
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			err := store.UpdateRegistration(context.Background(), &storage.Registration{
				ID:        test.id,
				FirstName: "Jimbo",
				LastName:  "Brown",
				Email:     "birds@flyfast.com",
