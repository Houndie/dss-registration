...
const Venue = ({title, address, days, google, children, image}) => (
	<>
		<h2>{title}</h2>
		<Container>
			<Row><Col>
				<ul>{days.map((day, idx) => (<li key={idx}>{day}</li>))}</ul>
				<p>{address.reduce((accumulator, currentValue) => (<>{accumulator}<br/>{currentValue}</>))}</p>
				{children}
			</Col><Col>
				<Image src={image}/>
			</Col></Row>
		</Container>
		<center><iframe title={title} class="embed-responsive-item" src={google} width="800" height="450" frameborder="0" style={{border:0}} allowfullscreen /></center>
	</>
)
